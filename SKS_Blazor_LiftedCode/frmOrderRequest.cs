...
namespace SKS
{
      ...
      : Gap.Blazor.Form
   {
      ...
      : base()
      {
         this.View = typeof(FrmOrderRequestView);
      ...
      private async Task cmdCustomers_Click(Object eventSender, EventArgs eventArgs)
      {
         await
         ...
         try
         {
            ...
            for ( modMain.i = 1; modMain.i <= tempForEndVar; modMain.i++ )
            {
               if ( Convert.ToString(fgProducts[modMain.i, 0].Value) != "0" )
               {
                  ...
                  modConnection.ExecuteSql($"Update Products Set UnitsOnOrder = UnitsOnOrder + {Convert.ToString(fgProducts[modMain.i, 0].Value)}" + $" Where ProductId = '{Convert.ToString(fgProducts[modMain.i, 1].Value)}'");
               }
            }
            editingData = false;
            if ( await Gap.Blazor.MessageBox.Show($"Order added successfully{Environment.NewLine}Would you like to add a new order?", "New data", Gap.Blazor.MessageBoxButtons.YesNo, Gap.Blazor.MessageBoxIcon.Question) == Gap.Blazor.DialogResult.Yes )
            {
               ClearFields();
            }
            else
      ...
      private async Task lvCustomers_ItemClick(Gap.Blazor.ListViewItem Item) => await RetrieveDataCustomer();

      private async Task RetrieveDataCustomer()
      {
         if ( editingData )
         {
         ...
         if ( !(lvCustomers.FocusedItem is null) )
         {
            ...
            currentCompanyName = Gap.Blazor.ListView.GetListViewSubItem(withVar, 1).Text;
