...
#region "Upgrade Support "
      ...
      public static frmActionOrderReception DefInstance
      {
         get
         {
                // For backward compatibility, keep synchronous property, but throw if called when instance needs to be created asynchronously.
            if ( m_vb6FormDefInstance is null || m_vb6FormDefInstance.IsDisposed )
            {
                    throw new InvalidOperationException("DefInstance must be initialized asynchronously. Use GetDefInstanceAsync().");
            }
